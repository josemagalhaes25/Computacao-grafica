...
// Função para criar e posicionar o plano com a textura no cenário
function adicionarTextoAoCenario(texto, position, scene) {
    const texture = criarTextoTextura(texto);
    const geometry = new THREE.PlaneGeometry(8, 2); // Ajusta o tamanho do plano
    const material = new THREE.MeshBasicMaterial({ map: texture, transparent: true });
    const plane = new THREE.Mesh(geometry, material);
    
    // Define a posição do plano
    plane.position.set(position.x, position.y, position.z);
    
    // Adiciona o plano ao cenário
    scene.add(plane);
}
...
// Adicionar textos ao cenário
adicionarTextoAoCenario('Resultado', { x: 0, y: 5, z: -10 }, scene);
adicionarTextoAoCenario('Dia', { x: -5, y: 3, z: -10 }, scene);
adicionarTextoAoCenario('Noite', { x: 0, y: 1, z: -10 }, scene);
adicionarTextoAoCenario('Rolar Dado', { x: 5, y: -1, z: -10 }, scene);
