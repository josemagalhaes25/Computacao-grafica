...
function criarNumeroTextura(numero) {
    ...
    canvas.width = 64; // Define a largura do canvas
    canvas.height = 64; // Define a altura do canvas
    context.font = '24px Arial'; // Define a fonte do texto
    context.fillStyle = 'black'; // Define a cor do texto
    context.fillText(numero.toString(), 10, 30); // Desenha o número no canvas
    const texture = new THREE.CanvasTexture(canvas); // Cria uma textura a partir do canvas
...
function criarEscadote1() {
    var materialDegrau = new THREE.MeshLambertMaterial({ color: 0x654321 }); // Cor castanha para os degraus
...
function criarCerca() {
    var materialPoste = new THREE.MeshLambertMaterial({ color: 0x8B4513 }); // Cor verde escuro para os postes
    var coresRipa = [0x8B4513, 0x8B4513, 0x8B4513]; // Cores para as ripas: amarelo, azul e vermelho
    ...
    // Criar postes nos cantos do tabuleiro
    criarPoste(-larguraTabuleiro / 2, -alturaTabuleiro / 2);
    criarPoste(larguraTabuleiro / 2, -alturaTabuleiro / 2);
    criarPoste(-larguraTabuleiro / 2, alturaTabuleiro / 2);
    criarPoste(larguraTabuleiro / 2, alturaTabuleiro / 2);

    // Criar postes adicionais ao longo dos lados
    for (var i = -larguraTabuleiro / 2 + 1; i < larguraTabuleiro / 2; i++) {
        criarPoste(i, -alturaTabuleiro / 2);
        criarPoste(i, alturaTabuleiro / 2);
    }
    for (var j = -alturaTabuleiro / 2 + 1; j < alturaTabuleiro / 2; j++) {
        criarPoste(-larguraTabuleiro / 2, j);
        criarPoste(larguraTabuleiro / 2, j);
    }

    // Criar ripas horizontais superiores e inferiores
    criarRipaHorizontal(-larguraTabuleiro / 2, -alturaTabuleiro / 2, larguraTabuleiro / 2, -alturaTabuleiro / 2);
    criarRipaHorizontal(-larguraTabuleiro / 2, alturaTabuleiro / 2, larguraTabuleiro / 2, alturaTabuleiro / 2);
    criarRipaHorizontal(-larguraTabuleiro / 2, -alturaTabuleiro / 2, -larguraTabuleiro / 2, alturaTabuleiro / 2);
    criarRipaHorizontal(larguraTabuleiro / 2, -alturaTabuleiro / 2, larguraTabuleiro / 2, alturaTabuleiro / 2);

    // Criar ripas verticais coloridas ao longo dos lados, exceto na entrada
    for (var k = -larguraTabuleiro / 2 + 0.5; k < larguraTabuleiro / 2; k += 0.5) {
        if (k < -0.5 || k > 0.5) { // Deixar espaço para a entrada na parte inferior central
            var corRipa = coresRipa[Math.floor(Math.random() * coresRipa.length)];
            criarRipaVertical(k, -alturaTabuleiro / 2, corRipa);
            criarRipaVertical(k, alturaTabuleiro / 2, corRipa);
        }
    }
    for (var l = -alturaTabuleiro / 2 + 0.5; l < alturaTabuleiro / 2; l += 0.5) {
        var corRipa2 = coresRipa[Math.floor(Math.random() * coresRipa.length)];
        criarRipaVertical(-larguraTabuleiro / 2, l, corRipa2);
        criarRipaVertical(larguraTabuleiro / 2, l, corRipa2);
    }
...
criarArvore({ x: -7, y: 0.5, z: -15 }, 2);
criarArvore({ x: 11, y: 0.5, z: -20 }, 2);
criarArvore({ x: 5, y: 0.5, z: -7 }, 2);
criarArvore({ x: -4.4, y: 0.5, z: -7 }, 2);
criarArvore({ x: 0, y: 0.5, z: -15 }, 2);
...
criarArvore({ x: -10, y: 0.5, z: 10 }, 2);
criarArvore({ x: 7, y: 0.5, z: 15 }, 2);
criarArvore({ x: -15, y: 0.5, z: 6 }, 2);
criarArvore({ x: 11, y: 0.5, z: 9 }, 2);


function carregarNuvens() {
    // Lista de posições onde as nuvens serão colocadas
    var posicoes = [
        { x: -5, y: 10, z: 0 },
        { x: -5, y: 10, z: 0.3 },
        { x: -5.3, y: 10, z: 0.2 },
...
function mostrarMenuInicial() {
    ...
    botaoIniciar.style.borderRadius = '20px'; // Define o raio das bordas como 20px
...
let mensagem; // Variável para armazenar a mensagem de vitória

function exibirMensagemVitoria() {
    mensagem = document.createElement('div');
    mensagem.id = 'mensagem-vitoria'; // Adicione um ID à mensagem
    ...
    mensagem.innerHTML = 'Parabéns! Você chegou ao final do tabuleiro! <br> Prima "J" para Jogar novamente!';
...
document.addEventListener('keydown', function(event) {
    if (event.key === 'J' || event.key === 'j') {
        // Alternar entre câmera perspectiva e ortográfica
        if (peao.position.x === posicaoFinal.x && peao.position.z === posicaoFinal.z) {
